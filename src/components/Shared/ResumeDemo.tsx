...
export function ResumeDemo() {
    ...
    const handleDownload = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        try {
            const response = await fetch('/jayesh_resume.pdf');
            console.log('File fetched successfully');
            const blob = await response.blob();
            const url = window.URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'jayesh_resume.pdf');
            router.push('/resume')
    ...
    return (
        <div className="md:w-10/12 w-11/12 mx-auto md:tracking-wide tracking-normal opacity-90 pb-1">
            ...
            <div className="md:p-3 p-2 border rounded-md my-10 overflow-y-scroll">
                <h1 className="sm:text-center text-center md:text-3xl text-xl font-normal mt-5 cursor-pointer" onClick={() => router.replace('/about')}>Berani Jayesh Abhalbhai</h1>
                <div className="sm:flex block space-y-4 sm:space-y-0 space-x-0 sm:space-x-2 md:space-x-4 cursor-pointer text-xs md:text-sm md:mt-20 mt-10 md:mb-2 mb-5 justify-center items-center ">
                    ...
                    <Link className="block" href="tel:+91 6355353728" >+91 6355353728 <IconPhone className="inline mb-0.5" size={16} /></Link>
                    <Separator orientation="vertical" className="hidden lg:flex" />
                    <Link className="block" href="mailto:jayeshberanii@gmail.com" >jayeshberanii@gmail.com <IconMail className="inline" size={16} /></Link>
                    <Separator orientation="vertical" className="hidden lg:flex" />
                    <Link className="block" target="_blank" href="https://github.com/jayeshberanii"><span className="mr-1 inline-flex">Github</span><IconBrandGithub className="inline" size={18} /></Link>
                    <Separator orientation="vertical" className="hidden lg:flex" />
                    <Link className="block" target="_blank" href="https://www.linkedin.com/in/jayesh-berani/"><span className="mr-1 inline-flex">LinkedIn</span><IconBrandLinkedin className="inline" size={20} /></Link>
                </div>
                <hr className="mb-10" />
                ...
                <div className="flex mt-4 text-xs md:text-sm justify-between items-start">
                    <h6 className="inline">College of AIT, Anand <span className="block mt-1 md:hidden ">(june 2019 - May 2023 )</span></h6>
                ...
                <div className="flex mt-2 justify-between items-start text-xs md:text-sm">
                    <h6 className="inline">Bachelor Of Technology <span className="block mt-1 md:hidden ">(CGPA - 7.83/10 )</span></h6>
                    <i className="md:inline hidden">CGPA - 7.83/10</i>
                </div>

                <hr className="my-10 w-11/12  " />

                <h5>Experience <IconCode className="inline " size={18} /><span className="block mt-1 text-xs md:text-sm ">  ( 2+ years )</span> </h5>
